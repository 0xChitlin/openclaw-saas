...
router.post("/", (req: Request, res: Response): void => {
  ...
  if (!customer) {
    ...
    dbRun("INSERT INTO customers (id, email, name, plan, stripe_id, status) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    dbRun("UPDATE customers SET plan = ?, stripe_id = COALESCE(?, stripe_id), status = 'active' WHERE id = ?",
  ...
  dbRun("INSERT INTO agents (id, customer_id, name, template, config, status) VALUES (?, ?, ?, ?, ?, ?)",
    [agentId, customer.id, agent_name || (agentTemplate + " agent"), agentTemplate, JSON.stringify(agentConfig), "paused"]);
  ...
  res.status(201).json({ success: true, customerId: customer.id, agentId, template: agentTemplate, status: "paused" });
});
